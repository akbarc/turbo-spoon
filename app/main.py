...
# Database imports - MUST import BEFORE pandas to avoid conflicts!
from database_pymssql import SQLServerConnection

# DO NOT import pandas here! database_pymssql has lazy import in execute_query()
# import pandas as pd

# AI Assistant imports - DISABLED for faster startup
...
# Customer Segmentation imports
from app.customer_segmentation import register_segmentation_routes

# Category Analysis imports
from app.category_analysis import register_category_routes
...
# Comprehensive Sales Analytics imports
from modules.sales_analytics_comprehensive import SalesAnalyticsComprehensive

# Sales Dashboard imports
from sales_dashboard.routes import sales_dashboard_bp
...
def init_database():
    ...
    try:
        ...
        db.connect()  # Actually connect BEFORE pandas is imported
...
def init_ai_assistant():
    ...
    # DISABLED: AI assistant causes 60 second timeout during startup
    # try:
    ...
    AI_AVAILABLE = False
    ai_assistant = None
    logger.info("⚠️  AI Assistant disabled for faster startup")
...
@app.route('/pos-system')
def pos_system_dashboard():
    """POS System Templates and Reports Dashboard"""
    return render_template('pos_system_dashboard.html')

@app.route('/106d')
def comprehensive_dashboard_106d():
    """Comprehensive Business Intelligence Dashboard - Ultimate Command Center"""
    return render_template('comprehensive_dashboard_106d.html')

@app.route('/table-builder')
def drag_drop_table_builder():
    """Drag & Drop Customizable Table Builder - Ultimate Data Exploration Tool"""
    return render_template('table_builder.html')


# Removed - AR dashboard route is now handled by ar_dashboard.py module
...
def customer_intelligence():
    ...
    try:
        ...
        # All Customers by Revenue in Period
        top_customers_query = """
        SELECT
            c.ID as customer_id,
...
def inventory_low_stock():
    ...
    try:
        ...
        # OPTIMIZED: Removed nested subquery, direct calculation, better indexes
        low_stock_query = """
        WITH SalesVelocity AS (
            -- Calculate average daily sales directly without nested subquery
                SELECT 
                    te.ItemID,
            ...
            JOIN dbo.TransactionEntry te WITH (NOLOCK)
                ...
                AND te.Quantity > 0  -- Only sales, not returns
            GROUP BY te.ItemID
            HAVING SUM(te.Quantity) > 0  -- Only items that are actively selling
        )
            SELECT 
                i.ID,
                i.Description,
        ...
        INNER JOIN SalesVelocity sv ON i.ID = sv.ItemID  -- INNER JOIN since we need sales data
        LEFT JOIN dbo.Category c WITH (NOLOCK) ON i.CategoryID = c.ID
            WHERE i.Inactive = 0
        AND i.Quantity > 0
        AND i.Quantity / (sv.avg_daily_sales * 7.0) < 2  -- Less than 2 weeks
...
def inventory_deadstock():
    ...
    try:
        ...
        # Get all deadstock items for display
        deadstock_query = """
        SELECT
            i.ID,
            i.Description,
...
def inventory_overstock():
    ...
    try:
        ...
        # OPTIMIZED: Combined both queries into one using window functions
        overstock_query = """
        WITH SalesVelocity AS (
            SELECT 
                te.ItemID,
        ...
        OverstockAnalysis AS (
            SELECT 
                ...
                CASE 
                    WHEN sv.avg_daily_sales > 0 AND i.Quantity / sv.avg_daily_sales > 42
                    THEN (i.Quantity - (sv.avg_daily_sales * 42)) * i.Cost
                    ELSE 0
                END as excess_value,
                -- Window functions for totals (eliminates second query)
                COUNT(*) OVER () as total_overstock_count,
                SUM(i.Quantity * i.Cost) OVER () as total_overstock_value,
                SUM(CASE
                    WHEN sv.avg_daily_sales > 0 AND i.Quantity / sv.avg_daily_sales > 42
                    THEN (i.Quantity - (sv.avg_daily_sales * 42)) * i.Cost
                    ELSE i.Quantity * i.Cost
                END) OVER () as total_excess_value
            FROM dbo.Item i WITH (NOLOCK)
            LEFT JOIN dbo.Category c WITH (NOLOCK) ON i.CategoryID = c.ID
            ...
            AND (
                ...
                (sv.avg_daily_sales > 0 AND i.Quantity / sv.avg_daily_sales > 42)
            )
        )
        SELECT
            ...
            total_overstock_count, total_overstock_value, total_excess_value
        FROM OverstockAnalysis
        ORDER BY excess_value DESC, inventory_value DESC
        OPTION (RECOMPILE)
        """

        # Execute single query (totals now included via window functions)
        ...
        if not result.empty:
            # Get totals from first row (window functions provide same value for all rows)
            total_overstock_value = float(result.iloc[0]['total_overstock_value'] or 0)
            total_overstock_count = int(result.iloc[0]['total_overstock_count'] or 0)
            total_excess_value = float(result.iloc[0]['total_excess_value'] or 0)
...
def ar_aging_analysis_optimized():
    ...
    try:
        ...
        SELECT * FROM ARData ORDER BY Balance DESC
        """
...
def inventory_alerts():
    ...
    try:
        ...
        # Get all low stock cigarettes and tobacco
        low_stock_query = """
        SELECT
            i.ID,
            i.Description,
...
def gp_opportunities():
    ...
    try:
        ...
        # Simplified opportunities analysis to prevent timeouts
        opportunities = []
        
        try:
            # Quick analysis of low margin categories
            ...
            SELECT TOP 5
                ...
                SUM(te.Price * te.Quantity) as Revenue,
                CAST(SUM(te.Price * te.Quantity - te.Cost * te.Quantity) * 100.0 / 
            ...
            WHERE {date_filter}
            GROUP BY cat.Name
            HAVING SUM(te.Price * te.Quantity) > 1000  -- Only significant categories
            ORDER BY Margin ASC
            """
            
            result = db.execute_query(low_margin_query, description="Low Margin Categories")
            ...
            if not result.empty:
                for _, row in result.iterrows():
                    ...
                    if margin < 20:  # Less than 20% margin
                        impact = revenue * 0.05  # 5% improvement potential
                        opportunities.append({
                            'opportunity': f'Improve {category} Margins',
                            'description': f'{category} category has {margin:.1f}% margin. Consider price adjustments or cost negotiations.',
                            'impact': impact,
                            'priority': 'High' if margin < 10 else 'Medium'
                        })
            
            # Add a general opportunity if we have data
            if opportunities:
                total_impact = sum(opp['impact'] for opp in opportunities)
                opportunities.insert(0, {
                    'opportunity': 'Category Margin Optimization',
                    'description': f'Focus on improving margins across {len(opportunities)} underperforming categories.',
                    'impact': total_impact,
                    'priority': 'High'
                })
            else:
                # Default opportunity when no specific issues found
                opportunities.append({
                    'opportunity': 'Maintain Strong Performance',
                    'description': 'All categories are performing well. Continue monitoring for optimization opportunities.',
                    'impact': 0,
                    'priority': 'Low'
                })
        
        except Exception as query_error:
            logger.warning(f"GP opportunities query error: {query_error}")
            # Fallback opportunity
            opportunities = [{
                'opportunity': 'Data Analysis Pending',
                'description': 'Detailed opportunity analysis is being processed. Check back later for insights.',
                'impact': 0,
                'priority': 'Medium'
            }]
...
def get_date_filter_for_timeframe(timeframe):
...
@app.route('/api/sales-analytics/executive-dashboard')
@with_db_lock
def sales_analytics_executive():
    """Get comprehensive executive sales dashboard with YoY comparisons"""
    try:
        if not db:
            return jsonify({'error': 'Database not initialized'}), 500
        
        timeframe = request.args.get('timeframe', 'YTD')
        analytics = SalesAnalyticsComprehensive(db)
        dashboard_data = analytics.get_executive_dashboard_data(timeframe)
        
        return jsonify(dashboard_data)
        
    except Exception as e:
        logger.error(f"Error in sales analytics executive dashboard: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/sales-analytics/monthly-trends')
@with_db_lock
def sales_analytics_monthly_trends():
    ...
    try:
        ...
        timeframe = request.args.get('timeframe', 'YTD')
        analytics = SalesAnalyticsComprehensive(db)
        trends_data = analytics.get_monthly_trends(timeframe)
...
@app.route('/api/sales-analytics/categories')
@with_db_lock
def sales_analytics_categories():
    ...
    try:
        ...
        analytics = SalesAnalyticsComprehensive(db)
        category_data = analytics.get_category_analysis(timeframe, limit)
...
@app.route('/api/sales-analytics/products')
@with_db_lock
def sales_analytics_products():
    ...
    try:
        ...
        analytics = SalesAnalyticsComprehensive(db)
        product_data = analytics.get_product_analysis(timeframe, limit)
...
@app.route('/api/sales-analytics/customers')
@with_db_lock
def sales_analytics_customers():
    ...
    try:
        ...
        analytics = SalesAnalyticsComprehensive(db)
        customer_data = analytics.get_customer_profitability(timeframe, limit)
        
        return jsonify(customer_data)
        
    except Exception as e:
        logger.error(f"Error in customer profitability: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/sales-analytics/excise-impact')
@with_db_lock
def sales_analytics_excise_impact():
    """Get excise tax impact analysis"""
    try:
        if not db:
            return jsonify({'error': 'Database not initialized'}), 500
        ...
        timeframe = request.args.get('timeframe', 'YTD')
        analytics = SalesAnalyticsComprehensive(db)
        excise_data = analytics.get_excise_tax_impact(timeframe)
        
        return jsonify(excise_data)
        
    except Exception as e:
        logger.error(f"Error in excise impact analysis: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/sales-analytics/insights')
@with_db_lock
def sales_analytics_insights():
    """Get performance insights and opportunities"""
    try:
        if not db:
            return jsonify({'error': 'Database not initialized'}), 500
        ...
        timeframe = request.args.get('timeframe', 'YTD')
        analytics = SalesAnalyticsComprehensive(db)
        insights = analytics.get_performance_insights(timeframe)
...
@app.route('/api/gp/yoy-comparison')
@with_db_lock
def gp_yoy_comparison():
    """Get year-over-year GP comparison"""
    try:
        if not db:
            return jsonify({'error': 'Database not initialized'}), 500
        ...
        gp_analyzer = GPAnalysis(db)
        current_data = gp_analyzer.get_executive_summary(timeframe)
        
        # Get previous year data by modifying the date filter
        # This is a simplified approach - in production you'd want more sophisticated date handling
        if timeframe == 'YTD':
            prev_start = f"{previous_year}-01-01"
            prev_end = f"{previous_year}-{datetime.now().strftime('%m-%d')}"
        elif timeframe == 'MTD':
            prev_month = datetime.now().replace(year=previous_year)
            prev_start = prev_month.replace(day=1).strftime('%Y-%m-%d')
            prev_end = prev_month.strftime('%Y-%m-%d')
        else:
            # For other timeframes, just get previous year same period
            prev_start = f"{previous_year}-01-01"
            prev_end = f"{previous_year}-12-31"
        
        # Custom query for previous year
        prev_query = f"""
        WITH PrevGPMetrics AS (
            SELECT 
                SUM(te.Price * te.Quantity) as TotalRevenue,
                SUM(te.Cost * te.Quantity) as TotalCost,
                SUM(te.Price * te.Quantity - te.Cost * te.Quantity) as TotalGP,
            ...
            LEFT JOIN Item i ON te.ItemID = i.ID
            LEFT JOIN Category cat ON i.CategoryID = cat.ID
            WHERE t.Time >= '{prev_start}' AND t.Time <= '{prev_end}'
        )
        SELECT 
            TotalRevenue as total_revenue,
            TotalCost as total_cost,
            TotalGP as total_gp,
            CAST(TotalGP * 100.0 / NULLIF(TotalRevenue, 0) as DECIMAL(5,2)) as gp_margin,
            TransactionCount as transaction_count,
            UniqueCustomers as active_customers
        FROM PrevGPMetrics
        """
        
        prev_result = db.execute_query(prev_query, description="Previous Year GP Data")
        
        if not prev_result.empty:
            prev_row = prev_result.iloc[0]
            previous_data = {
                'total_revenue': float(prev_row.get('total_revenue', 0)),
                'total_cost': float(prev_row.get('total_cost', 0)),
                'total_gp': float(prev_row.get('total_gp', 0)),
                'gp_margin': float(prev_row.get('gp_margin', 0)),
                'transaction_count': int(prev_row.get('transaction_count', 0)),
                'active_customers': int(prev_row.get('active_customers', 0))
            }
        else:
            previous_data = {}
        ...
        if previous_data and current_data:
            for key in ['total_revenue', 'total_cost', 'total_gp', 'gp_margin', 'transaction_count', 'active_customers']:
                current_val = current_data.get(key, 0)
                prev_val = previous_data.get(key, 0)
        ...
        logger.error(f"Error in YoY GP comparison: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/gp/monthly-breakdown')
@with_db_lock
def gp_monthly_breakdown():
    """Get monthly GP breakdown for current and previous year"""
    try:
        if not db:
            return jsonify({'error': 'Database not initialized'}), 500
        ...
        result = db.execute_query(query, description="Monthly GP Breakdown")
        ...
        logger.error(f"Error in monthly GP breakdown: {str(e)}")
        return jsonify({'error': str(e)}), 500

# ===================
# POS SYSTEM DASHBOARD ROUTES
# ===================

@app.route('/api/pos/receipts')
@with_db_lock
def get_pos_receipts():
    """Get all POS receipt templates"""
    try:
        query = """
        SELECT 
            ID,
        ...
        ORDER BY ID
        """
        receipts = db.execute_query(query)
        return jsonify({
            'success': True,
            'data': receipts.to_dict('records') if not receipts.empty else []
        })
    except Exception as e:
        logger.error(f"Error getting POS receipts: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/pos/reports')
@with_db_lock  
def get_pos_reports():
    """Get all POS report definitions"""
    try:
        query = """
        SELECT 
            ID,
        ...
        if not reports.empty:
            for _, row in reports.iterrows():
                ...
                filename = row['ReportFilename'].lower()
                if 'reganaly' in filename:
                    report_info['report_type'] = 'register_analysis'
                    report_info['category'] = 'Sales Analysis'
                elif 'labels' in filename:
                    report_info['report_type'] = 'customer_labels'
                    report_info['category'] = 'Customer Reports'
                else:
                    report_info['report_type'] = 'general'
                    report_info['category'] = 'General Reports'
                
                pos_reports.append(report_info)
        
        return jsonify({
            'success': True,
            'data': pos_reports,
            'total_reports': len(pos_reports)
        })
    except Exception as e:
        logger.error(f"Error getting POS reports: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/pos/run-crystal-report/<int:report_id>')
@with_db_lock
def run_crystal_report(report_id):
    """Run a specific Crystal Report from the POS system"""
    try:
        # Get report details
        report_query = """
        SELECT 
        ...
        report_result = db.execute_query(report_query, [report_id])
        
        if report_result.empty:
            return jsonify({'error': 'Report not found'}), 404
        
        report = report_result.iloc[0].to_dict()
        
        # Since these are Crystal Reports, we can't execute them directly
        # But we can provide the report information and suggest data queries
        ...
        report_data = {
            ...
            'status': 'Crystal Report - Cannot execute directly',
            'suggestion': 'This is a Crystal Report file. Use the custom reports below for similar data.'
        }
        
        # Provide equivalent data based on report type
        if 'reganaly' in report['ReportFilename'].lower():
            # Register Analysis - provide sales data
            sales_query = """
            SELECT
            ...
            equivalent_data = db.execute_query(sales_query)
            report_data['equivalent_data'] = equivalent_data.to_dict('records') if not equivalent_data.empty else []
            report_data['data_description'] = 'Register Analysis - Daily Sales Summary (Last 30 Days)'
            
        elif 'labels' in report['ReportFilename'].lower():
            # Customer Labels - provide customer data
            customer_query = """
            SELECT
            ...
            WHERE (Company IS NOT NULL AND Company != '') 
               OR (FirstName IS NOT NULL AND FirstName != '')
            ORDER BY COALESCE(Company, FirstName + ' ' + LastName)
            """
            
            equivalent_data = db.execute_query(customer_query)
            report_data['equivalent_data'] = equivalent_data.to_dict('records') if not equivalent_data.empty else []
            report_data['data_description'] = 'Customer Labels - Customer Contact Information'
        ...
        logger.error(f"Error running Crystal Report: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/pos/custom-buttons')
@with_db_lock
def get_pos_custom_buttons():
    """Get POS custom button configurations"""
    try:
        query = """
        SELECT 
            ID,
            ...
            Description,
            Style
        FROM dbo.CustomButtons
        ORDER BY Number
        """
        buttons = db.execute_query(query)
        return jsonify({
            'success': True,
            'data': buttons.to_dict('records') if not buttons.empty else []
        })
    except Exception as e:
        logger.error(f"Error getting custom buttons: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/pos/configuration')
@with_db_lock
def get_pos_configuration():
    ...
    try:
        ...
        """
        config = db.execute_query(query)
        return jsonify({
            'success': True,
            'data': config.to_dict('records')[0] if not config.empty else {}
        })
    except Exception as e:
        logger.error(f"Error getting POS configuration: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/pos/pole-display')
@with_db_lock
def get_pos_pole_display():
    """Get customer pole display messages"""
    try:
        query = """
        SELECT 
            ID,
            ...
            DateAndTimeLine1,
            DateAndTimeLine2
        FROM dbo.PoleDisplayMessage
        ORDER BY ID
        """
        messages = db.execute_query(query)
        return jsonify({
            'success': True,
            'data': messages.to_dict('records') if not messages.empty else []
        })
    except Exception as e:
        logger.error(f"Error getting pole display messages: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/pos/registers')
@with_db_lock
def get_pos_registers():
    """Get POS register configurations"""
    try:
        query = """
        SELECT 
            ID,
        ...
        ORDER BY Number
        """
        registers = db.execute_query(query)
        return jsonify({
            'success': True,
            'data': registers.to_dict('records') if not registers.empty else []
        })
    except Exception as e:
        logger.error(f"Error getting register configurations: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/pos/generate-receipt/<int:transaction_number>')
@app.route('/api/pos/generate-receipt/<int:transaction_number>/<int:template_id>')
@with_db_lock
def generate_pos_receipt(transaction_number, template_id=1):
    """Generate actual receipt for a transaction using POS templates"""
    try:
        ...
        transaction_result = db.execute_query(transaction_query, [transaction_number])
        
        if transaction_result.empty:
            return jsonify({'error': 'Transaction not found'}), 404
        
        transaction = transaction_result.iloc[0].to_dict()
        ...
        items_result = db.execute_query(items_query, [transaction_number])
        
        # Get actual POS receipt template
        template_query = """
        SELECT 
            Description,
            Title,
        ...
        template_result = db.execute_query(template_query, [template_id])
        
        # Get actual store information from Configuration table
        store_query = """
        SELECT 
            StoreName,
            StoreAddress1,
        ...
        store_result = db.execute_query(store_query)
        
        # Extract exact POS formatting from template
        business_name = "GEORGIA WHOLESALE"
        store_address = "2935-A NORTH DECATUR ROAD"
        store_city = "DECATUR, GA 30033"
        store_phone = "404-292-4899"
        
        if not store_result.empty:
            store = store_result.iloc[0]
            business_name = store['StoreName'] or business_name
            store_address = store['StoreAddress1'] or store_address
            store_city = f"{store['StoreCity']}, {store['StoreState']} {store['StoreZip']}" if store['StoreCity'] else store_city
            store_phone = store['StorePhone'] or store_phone
        
        # Extract footer messages from XML template
        footer_messages = []
        if not template_result.empty:
            template_xml = str(template_result.iloc[0]['TemplateSale'])
            import re
            
            # Extract comment lines from XML (including spaces in patterns)
            comment_patterns = [
                (r'<SET name="CommentLine1"[^>]*>"([^"]+)"</SET>', 'comment1'),
                (r'<SET name="CommentLine2"[^>]*>"([^"]+)"</SET>', 'comment2'), 
                (r'<SET name="CommentLine3"[^>]*>"([^"]+)"</SET>', 'comment3'),
                (r'<SET name="BCommentLine1"[^>]*>\s*"([^"]+)"\s*</SET>', 'bcomment1'),
                (r'<SET name="BCommentLine2"[^>]*>\s*"([^"]+)"\s*</SET>', 'bcomment2'),
                (r'<SET name="BCommentLine3"[^>]*>"([^"]+)"</SET>', 'bcomment3')
            ]
            
            for pattern, key in comment_patterns:
                match = re.search(pattern, template_xml)
                if match:
                    footer_messages.append(match.group(1).strip())
        
        # Generate receipt using EXACT POS format
        receipt_data = {
            'transaction': transaction,
            'items': items_result.to_dict('records') if not items_result.empty else [],
            'business_name': business_name,
            'store_address': store_address,
            'store_city': store_city,
            'store_phone': store_phone,
            'footer_messages': footer_messages,
            'template_format': 'EXACT_POS_XML',
            'template_id': template_id,
            'template_title': template_result.iloc[0]['Title'] if not template_result.empty else 'Unknown',
            'template_xml': template_result.iloc[0]['TemplateSale'] if not template_result.empty else None,
...
@app.route('/api/pos/generate-receipt-html/<int:transaction_number>')
@app.route('/api/pos/generate-receipt-html/<int:transaction_number>/<int:template_id>')
@with_db_lock
def generate_pos_receipt_html(transaction_number, template_id=1):
    """Generate HTML receipt using proper RMS template - optimized to match XML logic exactly"""
    try:
        with SQLServerConnection() as db:
            # OPTIMIZATION: Single query to get transaction + customer + store info
            # Matches XML template logic exactly for all fields
            main_query = """
            SELECT 
                -- Transaction fields (matches XML Transaction.*)
                t.TransactionNumber,
                t.Time,
                t.Total,
                t.SalesTax,
                ...
                -- Customer fields (matches XML Customer.*)
                COALESCE(c.Company, c.FirstName + ' ' + c.LastName) as CustomerName,
                c.PhoneNumber,
                c.Address,
                c.City,
                ...
                c.AccountNumber,
                c.TaxNumber,
                c.CustomText2 as CustomerTobaccoLic,
                c.AccountBalance as CustomerAccountBalance,
                c.PreviousBalance as CustomerPreviousBalance,

                -- Store fields (matches XML Store.*)
                COALESCE(cfg.StoreName, 'GEORGIA WHOLESALE') as StoreName,
                COALESCE(cfg.StoreAddress1, '2935-A NORTH DECATUR ROAD') as StoreAddress,
                COALESCE(cfg.StoreCity + ', ' + cfg.StoreState + ' ' + cfg.StoreZip, 'DECATUR, GA 30033') as StoreCityStateZip,
                COALESCE(cfg.StorePhone, '404-292-4899') as StorePhone
            FROM [dbo].[Transaction] t
            LEFT JOIN dbo.Customer c ON t.CustomerID = c.ID
            CROSS JOIN (SELECT TOP 1 * FROM dbo.Configuration) cfg
            WHERE t.TransactionNumber = %s
            """
            
            transaction_result = db.execute_query(main_query, [transaction_number])
            ...
            # OPTIMIZATION: Get line items with ALL fields needed (matches XML Entry.*)
            # Including Weight, UnitOfMeasure, and CORRECT excise tax from PUExciseEntry
            items_query = """
            SELECT 
                te.ID as EntryID,
                te.Quantity,
            ...
            # OPTIMIZATION: Get receipt template (only if needed for footer messages)
            template_xml = None
            template_query = """
            SELECT TemplateSale
            FROM dbo.Receipt 
            ...
            # Extract footer messages from template XML (matches XML template logic)
            footer_messages = []
            if template_xml:
                import re
                
                # Exact patterns from XML template SET definitions (lines 70-91)
                comment_patterns = [
                    r'<SET name="CommentLine1"[^>]*>"([^"]+)"</SET>',
                    r'<SET name="CommentLine2"[^>]*>"([^"]+)"</SET>',
                    r'<SET name="CommentLine3"[^>]*>"([^"]+)"</SET>',
                    r'<SET name="BCommentLine1"[^>]*>\s*"([^"]+)"\s*</SET>',
                    r'<SET name="BCommentLine2"[^>]*>\s*"([^"]+)"\s*</SET>',
                    r'<SET name="BCommentLine3"[^>]*>"([^"]+)"</SET>'
                ]
            ...
            # Calculate totals (matches XML template formulas)
            subtotal = float(transaction['Total']) - float(transaction['SalesTax'])
            tax = float(transaction['SalesTax'])
            total = float(transaction['Total'])
            
            # Format date/time (matches XML Transaction.Date and Transaction.Time)
            from datetime import datetime
            trans_time = datetime.fromisoformat(str(transaction['Time']).replace('Z', '+00:00'))
            transaction_date = trans_time.strftime('%m/%d/%Y')
            transaction_time = trans_time.strftime('%I:%M %p')
            
            # EXACT MATCH TO XML: Calculate total quantity (matches XML line 470)
            # "!!!!! Total " Transaction.TotalQuantity " QTY Items !!!!!"
            total_quantity = sum(float(item['Quantity']) for item in items_result.to_dict('records')) if not items_result.empty else 0
            
            # CORRECTED: Use PUExciseEntry.PriceC (ExciseTaxAmount) as the source of truth
            # PriceC contains the TOTAL excise tax for the line item from POS system
            excise_tax = 0.0
            items_list = []
            if not items_result.empty:
                for idx, item in enumerate(items_result.to_dict('records')):
                    # Use ExciseTaxAmount directly from PUExciseEntry (already calculated by POS)
                    item_excise = float(item.get('ExciseTaxAmount', 0) or 0)
                    excise_tax += item_excise

                    # Format item for template
                    items_list.append({
                        'LineNo': idx + 1,  # Matches XML LineNo counter
                        'Description': item['Description'],
                        'UnitPrice': item.get('UnitPrice', item['Price']),
                        'Weight': item.get('Weight', 1),
                        'UnitOfMeasure': item.get('UnitOfMeasure', 'EA'),
                        'Pack': f"{item.get('Weight', 1)} {item.get('UnitOfMeasure', 'EA')}",
                        'Quantity': item['Quantity'],
                        'Price': item['Price'],
                        'LineTotal': item['LineTotal']
                    })

            # Determine transaction type (matches XML Transaction.ReceiptTransactionName)
            transaction_type = "INVOICE"  # Default for transactionSales

            # Customer balance logic (matches XML ShowAccountBalance and ShowPreBalance logic)
            customer_balance = None
            previous_balance = None
            if transaction.get('CustomerID') and transaction.get('CustomerAccountBalance') is not None:
                customer_balance = float(transaction['CustomerAccountBalance'])
                previous_balance = float(transaction.get('CustomerPreviousBalance', 0) or 0)
            
            # Render the proper HTML template
            from flask import render_template
            return render_template('pos_receipt_proper.html',
                business_name=transaction['StoreName'],
                store_address=transaction['StoreAddress'],
                store_city=transaction['StoreCityStateZip'],
                store_phone=transaction['StorePhone'],
                transaction=transaction,
...
@app.route('/api/pos/get-all-reports')
@with_db_lock
def get_all_pos_reports():
    """Get all available POS reports including excise tax reports"""
    try:
        # Get Crystal Reports from Report table
        crystal_reports_query = """
        SELECT 
            ID,
            ReportFilename,
            Description,
        ...
        ORDER BY Description
        """
        crystal_reports = db.execute_query(crystal_reports_query)
        ...
        # Add Crystal Reports
        if not crystal_reports.empty:
            for _, row in crystal_reports.iterrows():
                all_reports.append({
                    'id': f"crystal_{row['ID']}",
                    'name': row['Description'],
                    'type': 'Crystal Report',
                    'category': 'POS System Reports',
                    'filename': row['ReportFilename'],
                    'can_run': False,
                    'description': 'Crystal Report - View data equivalent'
                })
        
        # Add ALL POS View Reports (25 enterprise views)
        view_reports = [
            # PUBLIC Views (Core POS Reports)
            {'id': 'public_transaction', 'name': 'Transaction Analysis', 'type': 'POS View', 'category': 'Sales Reports', 'view': 'PUBLIC_Transaction', 'description': 'Complete transaction analysis (225 fields)'},
            {'id': 'public_customer', 'name': 'Customer Master Report', 'type': 'POS View', 'category': 'Customer Reports', 'view': 'PUBLIC_Customer', 'description': 'Complete customer data (3,364 fields)'},
            {'id': 'public_item', 'name': 'Item Master Report', 'type': 'POS View', 'category': 'Inventory Reports', 'view': 'PUBLIC_Item', 'description': 'Complete item/inventory data (4,761 fields)'},
            {'id': 'public_category', 'name': 'Category Analysis', 'type': 'POS View', 'category': 'Sales Reports', 'view': 'PUBLIC_Category', 'description': 'Product category analysis'},
            {'id': 'public_cashier', 'name': 'Cashier Performance Report', 'type': 'POS View', 'category': 'Employee Reports', 'view': 'PUBLIC_Cashier', 'description': 'Employee performance (324 fields)'},
            {'id': 'public_supplier', 'name': 'Supplier Analysis', 'type': 'POS View', 'category': 'Supplier Reports', 'view': 'PUBLIC_Supplier', 'description': 'Supplier data (361 fields)'},
            {'id': 'public_tender', 'name': 'Payment Method Analysis', 'type': 'POS View', 'category': 'Financial Reports', 'view': 'PUBLIC_Tender', 'description': 'Payment methods (441 fields)'},
            {'id': 'public_tax', 'name': 'Tax Analysis Report', 'type': 'POS View', 'category': 'Tax Reports', 'view': 'PUBLIC_Tax', 'description': 'Tax calculations (484 fields)'},
            
            # NOTE: Broken excise/movement/tender views REMOVED (caused 60-second timeouts)
            # - PUVIEWEXCISECOLLECT, PUVIEWEXCISEPAID, PUVIEWEXCISETRANSACTION (7 views)
            # - VIEWEXCISETAXCOLLECT, VIEWEXCISETAXPAID, VIEWHOLDEXCISETAX, VIEWPOEXCISETAX
            # - VIEWITEMMOVEMENT, VIEWITEMMOVEMENTHISTORY, VIEWTENDERS (3 views)
            # All excise functionality moved to fast PUExciseEntry table queries (see excise_reports below)
        ]
        
        # Add Table-Based Reports (using actual POS tables)
        table_reports = [
            {'id': 'daily_sales_table', 'name': 'Daily Sales (POS Table)', 'type': 'POS Table', 'category': 'Sales Reports', 'table': 'DailySales', 'description': 'Pre-calculated daily sales from POS'},
            {'id': 'batch_report', 'name': 'Batch Analysis', 'type': 'POS Table', 'category': 'Operations Reports', 'table': 'Batch', 'description': 'Transaction batch analysis'},
            {'id': 'audit_log', 'name': 'System Audit Log', 'type': 'POS Table', 'category': 'Audit Reports', 'table': 'AuditLog', 'description': 'Complete system audit trail'},
            {'id': 'ar_history', 'name': 'AR History Report', 'type': 'POS Table', 'category': 'Financial Reports', 'table': 'AccountReceivableHistory', 'description': 'Complete AR transaction history'},
            {'id': 'inventory_transfers', 'name': 'Inventory Transfer Log', 'type': 'POS Table', 'category': 'Inventory Reports', 'table': 'InventoryTransferLog', 'description': 'Inventory movement tracking'},
            {'id': 'item_value_log', 'name': 'Item Value Changes', 'type': 'POS Table', 'category': 'Inventory Reports', 'table': 'ItemValueLog', 'description': 'Item price/cost change history'},
            {'id': 'order_history', 'name': 'Order History Report', 'type': 'POS Table', 'category': 'Operations Reports', 'table': 'OrderHistory', 'description': 'Complete order tracking'},
            {'id': 'visa_net_batch', 'name': 'Credit Card Batch Report', 'type': 'POS Table', 'category': 'Financial Reports', 'table': 'VisaNetBatch', 'description': 'Credit card processing batches'},
        ]
        
        # Add Custom Analysis Reports
        analysis_reports = [
            # Existing reports
            {'id': 'sales_by_category', 'name': 'Sales by Category', 'type': 'Analysis Report', 'category': 'Sales Reports', 'description': 'Revenue breakdown by product category'},
            {'id': 'sales_by_item', 'name': 'Sales by Item', 'type': 'Analysis Report', 'category': 'Sales Reports', 'description': 'Top selling items analysis'},
            {'id': 'customer_sales_analysis', 'name': 'Customer Sales Analysis', 'type': 'Analysis Report', 'category': 'Customer Reports', 'description': 'Customer purchase patterns'},
            {'id': 'inventory_valuation', 'name': 'Inventory Valuation', 'type': 'Analysis Report', 'category': 'Inventory Reports', 'description': 'Current inventory values'},
            {'id': 'profit_analysis', 'name': 'Profit Analysis', 'type': 'Analysis Report', 'category': 'Financial Reports', 'description': 'Gross profit analysis with tobacco uplifts'},
            {'id': 'cashier_performance', 'name': 'Cashier Performance', 'type': 'Analysis Report', 'category': 'Employee Reports', 'description': 'Employee sales performance'},
            {'id': 'payment_methods', 'name': 'Payment Methods', 'type': 'Analysis Report', 'category': 'Financial Reports', 'description': 'Payment method breakdown'},
            {'id': 'ar_aging', 'name': 'AR Aging Report', 'type': 'Analysis Report', 'category': 'Financial Reports', 'description': 'Accounts receivable aging'},

            # NEW - Sales Reports
            {'id': 'sales_by_department', 'name': 'Sales by Department', 'type': 'Analysis Report', 'category': 'Sales Reports', 'description': 'Revenue breakdown by department'},
            {'id': 'top_items', 'name': 'Top Items Report', 'type': 'Analysis Report', 'category': 'Sales Reports', 'description': 'Top 100 selling items ranked by revenue'},
            {'id': 'top_customers', 'name': 'Top Customers Report', 'type': 'Analysis Report', 'category': 'Customer Reports', 'description': 'Top 100 customers ranked by total purchases'},

            # NEW - Transaction Reports
            {'id': 'transaction_detail', 'name': 'Transaction Detail Report', 'type': 'Analysis Report', 'category': 'Sales Reports', 'description': 'Detailed line-by-line transaction breakdown'},
            {'id': 'return_report', 'name': 'Returns & Refunds Report', 'type': 'Analysis Report', 'category': 'Sales Reports', 'description': 'All returns and refunds with details'},
            {'id': 'void_report', 'name': 'Voided Transactions Report', 'type': 'Analysis Report', 'category': 'Audit Reports', 'description': 'All voided transactions with reasons'},

            # NEW - Inventory Reports
            {'id': 'inventory_list', 'name': 'Complete Inventory List', 'type': 'Analysis Report', 'category': 'Inventory Reports', 'description': 'Full inventory listing with stock status'},
            {'id': 'reorder_report', 'name': 'Reorder Report', 'type': 'Analysis Report', 'category': 'Inventory Reports', 'description': 'Items below reorder point with supplier info'},
            {'id': 'physical_count_worksheet', 'name': 'Physical Count Worksheet', 'type': 'Analysis Report', 'category': 'Inventory Reports', 'description': 'Physical inventory count worksheet'},

            # NEW - Financial Reports
            {'id': 'cash_drawer_report', 'name': 'Cash Drawer Report', 'type': 'Analysis Report', 'category': 'Financial Reports', 'description': 'Cash drawer reconciliation by batch'},
            {'id': 'tax_summary', 'name': 'Tax Summary Report', 'type': 'Analysis Report', 'category': 'Tax Reports', 'description': 'Tax collected by tax type'},
            {'id': 'tender_types_detail', 'name': 'Tender Types Detail', 'type': 'Analysis Report', 'category': 'Financial Reports', 'description': 'Enhanced payment method analysis with statistics'},
            {'id': 'discount_report', 'name': 'Discount Report', 'type': 'Analysis Report', 'category': 'Financial Reports', 'description': 'All discounts given with amounts and percentages'},
        ]
        ...
        return jsonify({
            ...
            'categories': list(set([r['category'] for r in all_reports]))
        })
...
@with_db_lock
def run_pos_report():
    """Run POS reports with custom filters and pagination"""
    try:
        ...
        # Pagination parameters - default to no limit (show all)
        ...
        date_column_map = {
            ...
            'customer_sales': 't.Time',
            'item_sales': 't.Time',
            'cashier_performance': 't.Time',
            'payment_methods': 't.Time',
            'hourly_sales': 't.Time',
            'register_analysis': 't.Time',
            'sales_by_category': 't.Time',
            'sales_by_item': 't.Time',
            'profit_analysis': 't.Time',
            'sales_by_department': 't.Time',
            'top_items': 't.Time',
            'top_customers': 't.Time',
            'transaction_detail': 't.Time',
            'return_report': 't.Time',
            'tax_summary': 't.Time',
            'void_report': 't.Time',
            'tender_types_detail': 't.Time',
            'discount_report': 't.Time',
            # PUExciseEntry reports (use pue.TransactionTime)
            'pu_excise_summary': 'pue.TransactionTime',
            'excise_by_category': 'pue.TransactionTime',
            'excise_transactions': 'pue.TransactionTime',
            'daily_excise': 'pue.TransactionTime',
            'excise_simple': 'pue.TransactionTime',
        ...
        date_column = date_column_map.get(report_type, 't.Time')  # Default to t.Time for backwards compatibility
        ...
        if report_type == 'daily_sales':
            ...
            FROM [dbo].[Transaction] t
            LEFT JOIN dbo.Cashier cash ON t.CashierID = cash.ID
            WHERE 1=1 {date_filter} {additional_filters}
            GROUP BY t.CashierID, cash.Name
            ORDER BY TotalSales DESC
            """
            ...
            query = f"""
            SELECT 
                te.Description as PaymentMethod,
        ...
        elif report_type == 'pu_excise_summary':
            # Use POS system PUExciseEntry table as source of truth
            query = f"""
            SELECT 
        ...
        elif report_type == 'excise_by_category':
            # ORIGINAL PUExciseEntry query - will fail if table doesn't exist
            query = f"""
            SELECT 
                cat.Name as Category,
                COUNT(DISTINCT pue.TransactionNumber) as TransactionCount,
        ...
        elif report_type == 'excise_transactions':
            # ORIGINAL PUExciseEntry query - will fail if table doesn't exist
            query = f"""
            SELECT
            ...
            SELECT 
                ...
                SUM(CASE WHEN DATEDIFF(day, ar.Date, GETDATE()) <= 30 THEN ar.Balance ELSE 0 END) as [Current],
                SUM(CASE WHEN DATEDIFF(day, ar.Date, GETDATE()) BETWEEN 31 AND 60 THEN ar.Balance ELSE 0 END) as Days31_60,
                SUM(CASE WHEN DATEDIFF(day, ar.Date, GETDATE()) BETWEEN 61 AND 90 THEN ar.Balance ELSE 0 END) as Days61_90,
                SUM(CASE WHEN DATEDIFF(day, ar.Date, GETDATE()) > 90 THEN ar.Balance ELSE 0 END) as Over90,
                MAX(DATEDIFF(day, ar.Date, GETDATE())) as OldestInvoiceDays
            FROM dbo.AccountReceivable ar
        ...
        # Use ACTUAL POS report logic - DailySales table (pre-calculated by POS)
        elif report_type == 'daily_sales_pos':
            query = f"""
            SELECT 
                Date,
                CASE 
                    WHEN Type = 1 THEN 'Sales'
                    WHEN Type = 2 THEN 'Returns' 
                    WHEN Type = 3 THEN 'Voids'
                    WHEN Type = 4 THEN 'Payments'
                    ELSE 'Other'
        ...
        # Simple working reports using base tables only
        elif report_type == 'register_analysis':
            # This mimics the RegAnaly.def Crystal Report
            query = f"""
            SELECT 
                CAST(t.Time AS DATE) as SaleDate,
                COUNT(DISTINCT t.TransactionNumber) as TransactionCount,
        ...
        elif report_type == 'customer_labels':
            # This mimics the Labels.def Crystal Report
            query = f"""
            SELECT
                COALESCE(Company, FirstName + ' ' + LastName) as CustomerName,
                AccountNumber,
        ...
        # Simple excise report using PUExciseEntry table
        ...
        elif report_type == 'daily_sales_table':
            query = f"""
            SELECT 
                Date, Type, Total,
                CASE 
        ...
        elif report_type == 'ar_history':
            query = f"""
            SELECT
                arh.Date, arh.Amount, arh.Comment,
            ...
            SELECT 
                ...
                SUM(i.Quantity * i.Cost) as TotalCostValue,
                SUM(i.Quantity * i.Price) as TotalRetailValue,
                SUM(i.Quantity) as TotalQuantity,
                AVG(i.Price) as AvgRetailPrice,
                AVG(i.Cost) as AvgCost
            FROM dbo.Item i
            LEFT JOIN dbo.Category cat ON i.CategoryID = cat.ID
            WHERE i.Quantity > 0 AND i.Inactive = 0
            GROUP BY cat.Name
        ...
        elif report_type == 'sales_by_item':
            query = f"""
            SELECT
                i.Description as ItemName,
                i.ItemLookupCode,
            ...
            SELECT 
                ...
                (SUM(te.Price * te.Quantity) - SUM(
                    CASE 
                        ...
                        ELSE te.Cost * te.Quantity
                    END
                )) / SUM(te.Price * te.Quantity) * 100 as ProfitMargin
            FROM [dbo].[Transaction] t
            JOIN dbo.TransactionEntry te ON t.TransactionNumber = te.TransactionNumber
            JOIN dbo.Item i ON te.ItemID = i.ID
            LEFT JOIN dbo.Category cat ON i.CategoryID = cat.ID
            ...
            query = f"""
            SELECT TOP 100
                ROW_NUMBER() OVER (ORDER BY SUM(te.Price * te.Quantity) DESC) as Rank,
                i.Description as ItemName,
                i.ItemLookupCode,
                cat.Name as Category,
                dept.Name as Department,
            ...
            LEFT JOIN dbo.Department dept ON i.DepartmentID = dept.ID
            WHERE 1=1 {date_filter} {additional_filters}
            GROUP BY i.ID, i.Description, i.ItemLookupCode, cat.Name, dept.Name
            ORDER BY TotalRevenue DESC
            """
            ...
            query = f"""
            SELECT TOP 100
                ROW_NUMBER() OVER (ORDER BY SUM(t.Total) DESC) as Rank,
                COALESCE(c.Company, c.FirstName + ' ' + c.LastName) as CustomerName,
                c.AccountNumber,
                c.PhoneNumber,
                c.EmailAddress,
                ...
                DATEDIFF(day, MIN(t.Time), MAX(t.Time)) as CustomerTenureDays
            FROM [dbo].[Transaction] t
            LEFT JOIN dbo.Customer c ON t.CustomerID = c.ID
            WHERE 1=1 {date_filter} {additional_filters}
            GROUP BY c.ID, c.Company, c.FirstName, c.LastName, c.AccountNumber, c.PhoneNumber, c.EmailAddress
            ORDER BY TotalSales DESC
            """

        elif report_type == 'transaction_detail':
            query = f"""
            SELECT
                t.TransactionNumber,
            ...
            LEFT JOIN dbo.Cashier cash ON t.CashierID = cash.ID
            WHERE 1=1 {date_filter} {additional_filters}
            ORDER BY t.Time DESC, t.TransactionNumber, te.ID
            """
            ...
            SELECT
                t.TransactionNumber,
                t.Time as ReturnTime,
                COALESCE(c.Company, c.FirstName + ' ' + c.LastName) as CustomerName,
                cash.Name as CashierName,
                i.Description as ItemName,
                i.ItemLookupCode,
                cat.Name as Category,
                te.Quantity as ReturnQuantity,
                te.Price as ReturnPrice,
                te.Price * te.Quantity as ReturnAmount,
                t.Total as TransactionTotal
            FROM [dbo].[Transaction] t
            JOIN dbo.TransactionEntry te ON t.TransactionNumber = te.TransactionNumber
            JOIN dbo.Item i ON te.ItemID = i.ID
            ...
            LEFT JOIN dbo.Customer c ON t.CustomerID = c.ID
            LEFT JOIN dbo.Cashier cash ON t.CashierID = cash.ID
            WHERE (te.Quantity < 0 OR t.Total < 0) {date_filter} {additional_filters}
            ORDER BY t.Time DESC
            """

        elif report_type == 'inventory_list':
            query = f"""
            SELECT
                i.ItemLookupCode,
                i.Description as ItemName,
                ...
                i.Cost,
                i.Price - i.Cost as UnitProfit,
                i.Quantity * i.Price as RetailValue,
                i.Quantity * i.Cost as CostValue,
                i.ReorderPoint,
                i.RestockLevel,
                CASE
                    WHEN i.Quantity <= i.ReorderPoint THEN 'Reorder Now'
                    WHEN i.Quantity <= i.RestockLevel THEN 'Low Stock'
                    ELSE 'In Stock'
                END as StockStatus
            FROM dbo.Item i
            LEFT JOIN dbo.Category cat ON i.CategoryID = cat.ID
        ...
        elif report_type == 'cash_drawer_report':
            query = f"""
            SELECT
                b.BatchNumber,
            ...
            LEFT JOIN dbo.[Transaction] t ON t.BatchNumber = b.BatchNumber
            WHERE 1=1 {date_filter.replace('t.Time', 'b.OpeningTime')}
            GROUP BY b.BatchNumber, b.OpeningTime, b.ClosingTime, r.Description,
                     b.OpeningTotal, b.ClosingTotal, b.Dropped, b.PaidOut
            ...
            LEFT JOIN dbo.Cashier cash ON t.CashierID = cash.ID
            LEFT JOIN dbo.TransactionEntry te ON t.TransactionNumber = te.TransactionNumber
            WHERE (t.Status < 0 OR t.Comment LIKE '%void%' OR t.Comment LIKE '%cancel%') {date_filter} {additional_filters}
            GROUP BY t.TransactionNumber, t.Time, c.Company, c.FirstName, c.LastName,
                     cash.Name, t.Total, t.Comment, t.Status
            ORDER BY t.Time DESC
            """

        elif report_type == 'tender_types_detail':
            query = f"""
            SELECT
                ...
                SUM(te.Amount) as TotalAmount,
                AVG(te.Amount) as AvgAmount,
                MIN(te.Amount) as MinAmount,
                MAX(te.Amount) as MaxAmount,
                SUM(CASE WHEN te.Amount > t.Total THEN te.Amount - t.Total ELSE 0 END) as TotalChange
            FROM [dbo].[Transaction] t
            JOIN dbo.TenderEntry te ON t.TransactionNumber = te.TransactionNumber
        ...
        elif report_type == 'discount_report':
            query = f"""
            SELECT
                ...
                te.Quantity,
                (te.FullPrice - te.Price) * te.Quantity as TotalDiscount
            FROM [dbo].[Transaction] t
            JOIN dbo.TransactionEntry te ON t.TransactionNumber = te.TransactionNumber
            JOIN dbo.Item i ON te.ItemID = i.ID
            LEFT JOIN dbo.Customer c ON t.CustomerID = c.ID
            LEFT JOIN dbo.Cashier cash ON t.CashierID = cash.ID
            WHERE te.FullPrice > te.Price AND te.FullPrice > 0 {date_filter} {additional_filters}
            ORDER BY t.Time DESC
            """

        elif report_type == 'reorder_report':
            query = f"""
            SELECT
                i.ItemLookupCode,
                i.Description as ItemName,
                ...
                i.ReorderPoint,
                i.RestockLevel,
                i.ReorderPoint - i.Quantity as QuantityNeeded,
                i.Cost,
                (i.ReorderPoint - i.Quantity) * i.Cost as ReorderCost,
                s.SupplierName,
                s.ContactName,
                s.PhoneNumber
            FROM dbo.Item i
        ...
        elif report_type == 'physical_count_worksheet':
            query = f"""
            SELECT
                i.ItemLookupCode,
                i.Description as ItemName,
            ...
            LEFT JOIN dbo.Department dept ON i.DepartmentID = dept.ID
            WHERE i.Quantity > 0
            ORDER BY dept.Name, cat.Name, i.Description
            """

        else:
            return jsonify({'error': 'Invalid report type'}), 400
        
        # Add pagination using OFFSET/FETCH if limit is specified
        # This applies to all queries uniformly
        if limit is not None:
            # SQL Server OFFSET/FETCH requires ORDER BY
            # The query already has ORDER BY, so we just append OFFSET/FETCH
        ...
        return jsonify({
            ...
            'pagination': {
                'limit': limit,
                'offset': offset,
                'returned_rows': len(result) if not result.empty else 0
            },
        ...
        logger.error(f"Error running POS report: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/pos/search-transactions')
@with_db_lock
def search_pos_transactions():
    ...
    try:
        ...
        if start_date and end_date:
            where_conditions.append("t.Time >= %s AND t.Time < DATEADD(day, 1, %s)")
            params.extend([start_date, end_date])
        elif start_date:
            where_conditions.append("CAST(t.Time AS DATE) = %s")
            params.append(start_date)
            
        if min_amount is not None:
            where_conditions.append("t.Total >= %s")
        ...
        if customer_id:
            where_conditions.append("t.CustomerID = %s")
            params.append(customer_id)
            
        if search_term:
            where_conditions.append("(c.Company LIKE %s OR c.FirstName LIKE %s OR c.LastName LIKE %s OR t.Comment LIKE %s OR t.ReferenceNumber LIKE %s)")
...
@app.route('/api/pos/export-report')
@with_db_lock
def export_pos_report():
    """Export POS report as CSV or Excel"""
    try:
        ...
        # Get the report data (reuse the run_pos_report logic)
        from flask import current_app
        with current_app.test_request_context(f'/api/pos/run-report?type={report_type}&start_date={start_date}&end_date={end_date}'):
            report_response = run_pos_report()
        ...
        if hasattr(report_response, 'get_json'):
            ...
            if export_format == 'excel':
                ...
                filename = f'pos_report_{report_type}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx'
                return send_file(
                    output,
                    as_attachment=True,
                ...
                response = Response(csv_data, mimetype='text/csv')
                filename = f'pos_report_{report_type}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
...
@with_db_lock
def execute_custom_table_query():
    """Execute custom table builder query with dynamic fields and filters"""
    try:
        ...
        # Use existing POS report logic but with custom field selection
        # This leverages our existing 33 working reports
        query_params = {
            'type': data_source,
            'start_date': start_date,
            'end_date': end_date
        ...
        # Execute using existing POS report system
        with app.test_request_context(f'/api/pos/run-report?{urlencode(query_params)}'):
            from flask import request as flask_request
            result = run_pos_report()
            
        # If result is successful, filter fields and apply custom filtering
        if hasattr(result, 'get_json') and result.get_json().get('success'):
            ...
            if selected_fields:
                filtered_data = []
                for row in data:
                    filtered_row = {field: row.get(field) for field in selected_fields}
            ...
            if filters:
                for filter_config in filters:
                    ...
                    if field and operator and value:
                        data = apply_custom_filter(data, field, operator, value)
            
            # Apply custom sorting
            if sort_field:
                data = apply_custom_sort(data, sort_field, sort_direction)
            ...
            return jsonify({
                ...
                'fields': selected_fields or 'all',
                'filters_applied': len(filters),
                'execution_time': '< 1s'
            })
        else:
            return jsonify({'error': 'Failed to execute base query'}), 500
            
    except Exception as e:
        logger.error(f"Error executing custom table query: {str(e)}")
        return jsonify({'error': str(e)}), 500

def apply_custom_filter(data, field, operator, value):
    ...
    for row in data:
        ...
        if operator == 'equals':
            if str(row_value).lower() == value.lower():
                filtered_data.append(row)
        elif operator == 'contains':
            if value.lower() in str(row_value).lower():
                filtered_data.append(row)
...
def apply_custom_sort(data, sort_field, sort_direction):
    """Apply custom sorting to data"""
    try:
        def sort_key(row):
            value = row.get(sort_field)
            if value is None:
                return 0 if sort_direction == 'ASC' else float('inf')
...
@app.route('/api/pos/daily-summary')
@with_db_lock
def get_pos_daily_summary():
    """Get daily POS summary with key metrics"""
    try:
        ...
        SELECT 
            ...
            AVG(t.Total) as AvgTransaction,
            COUNT(DISTINCT t.CustomerID) as UniqueCustomers,
            COUNT(DISTINCT t.CashierID) as ActiveCashiers,
            MIN(t.Time) as FirstTransaction,
            MAX(t.Time) as LastTransaction
        FROM [dbo].[Transaction] t
        WHERE CAST(t.Time AS DATE) = %s
        """
        
        summary_result = db.execute_query(summary_query, [date])
        
        # All categories for the day
        categories_query = f"""
        SELECT
            cat.Name as Category,
            SUM(te.Price * te.Quantity) as Revenue,
        ...
        categories_result = db.execute_query(categories_query, [date])
        ...
        payments_result = db.execute_query(payments_query, [date])
        
        return jsonify({
            'success': True,
            'date': date,
            'summary': summary_result.to_dict('records')[0] if not summary_result.empty else {},
            'top_categories': categories_result.to_dict('records') if not categories_result.empty else [],
            'payment_methods': payments_result.to_dict('records') if not payments_result.empty else []
        })
...
register_segmentation_routes(app, db)
...
# Register sales dashboard blueprint
app.register_blueprint(sales_dashboard_bp)

# Register sales dashboard V2 with all advanced features
from sales_dashboard_v2.routes import dashboard_v2

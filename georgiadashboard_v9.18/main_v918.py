...
from ar_dashboard import register_ar_routes

# Cohort Analysis imports
from cohort_dashboard import register_cohort_routes

# Customer Ledger imports
from customer_ledger_api import register_ledger_routes
...
# Global database connection with caching
db = None
...
def with_db_lock(func):
    """Decorator to ensure thread-safe database operations with caching"""
...
def cached_query(cache_key, ttl=CACHE_TTL):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if cache_key in query_cache and cache_key in cache_timestamps:
                if time.time() - cache_timestamps[cache_key] < ttl:

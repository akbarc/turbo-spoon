...
class GPAnalysis:
    ...
    def get_executive_summary(self, timeframe='YTD'):
        ...
        WITH GPMetrics AS (
            SELECT 
                -- Simplified GP metrics for better performance (excise tax calculated separately if needed)
...
te.Cost * te.Quantity) * 100.0 / NULLIF(SUM(te.Price * te.Quantity), 0) as DECIMAL(10,2)) as Margin
            FROM [dbo].[Transaction] t
            JOIN TransactionEntry te ON te.TransactionNumber = t.TransactionNumber
            JOIN Item i ON te.ItemID = i.ID
            LEFT JOIN Category cat ON i.CategoryID = cat.ID

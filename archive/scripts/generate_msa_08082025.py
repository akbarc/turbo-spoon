#!/usr/bin/env python3
"""
Generate MSA file for 08/08/2025 using dynamic UPC matching and POS data
"""
...
def generate_msa_file(prior_records, sales_data, upc_mapping, output_file):
    ...
    with open(output_file, 'w') as f:
        # Write updated header (change date from 20250801 to 20250808)
        if prior_records['header']:
            header = prior_records['header'].replace('20250801', '20250808')
            f.write(header + '\n')
        ...
        for customer_id, customer_sales in sorted(sales_data.items()):
            for upc, quantity in sorted(customer_sales.items()):
                if quantity > 0:
                    pur_line = format_pur_record(customer_id, upc, quantity, '20250808')
                    f.write(pur_line + '\n')
...
def main():
    # File paths
    prior_msa_file = "MSA Data Fr/08012025"
    output_file = "generated_msa_08082025.txt"
